...
│       │   └── quality-rules.md    # 산출 직전 셀프 리뷰 체크리스트 (의미적 판단만)
│       └── scripts/
│           ├── validate_prd.py     # 구조 린터 (Step 4에서 자동 실행, stdlib만 사용)
...
Use the prd-maker skill (${CLAUDE_PLUGIN_ROOT}/skills/prd-maker/SKILL.md) to interview the user about their product idea and generate PRD.md.
...
### Task 9: PRD 구조 린터 (scripts/validate_prd.py)

**Files:**
- Create: `skills/prd-maker/scripts/validate_prd.py`
- Create: `skills/prd-maker/scripts/test_validate_prd.py`
- Modify: `skills/prd-maker/SKILL.md` (Step 4)
- Modify: `skills/prd-maker/references/quality-rules.md` (범위 주석 추가)

**배경:** `quality-rules.md`의 5개 셀프 리뷰 항목 중 구조적인 것들(섹션 존재, non-goals 개수, 체크박스 형식)은 결정론적이라 LLM 셀프 리뷰보다 스크립트가 더 싸고 신뢰도가 높다. 의미적 판단(예: "이 수용 기준이 기계 검증 가능한가")은 여전히 LLM 몫으로 남긴다. 산출되는 PRD는 사용자 언어로 작성되므로, 모든 체크는 언어에 무관해야 한다 (헤딩 단어가 아닌 숫자·마크다운 구조만 매칭).

- [x] **Step 1: 테스트 먼저 작성 (TDD)** — `test_validate_prd.py`에 stdlib `unittest` 기반 테스트 작성: 유효한 PRD 픽스처(7섹션+헤더+non-goals 3개+체크박스 있는 phase 2개) → exit 0 전부 PASS; 섹션 누락(`## 4.` 없음) → CHECK 2 실패; non-goals 2개 → CHECK 3 실패; 체크박스 없는 phase → CHECK 4 실패; phase 내 51개 번호 요구사항 → CHECK 5 실패; blockquote 헤더 없음 → CHECK 1 실패; `(가정)`/`(assumption)` 둘 다 정확한 개수로 집계. 스크립트가 없는 상태에서 실행해 전부 실패하는 것을 확인.

- [x] **Step 2: `validate_prd.py` 구현** — Python 3 stdlib(`re`, `sys`, `pathlib`)만 사용. `python3 validate_prd.py <path-to-PRD.md>` 형태. 5개 체크:
  1. **에이전트 헤더**: 첫 5개 비어있지 않은 줄 안에 `> `로 시작하는 blockquote 존재
  2. **7개 번호 섹션**: `^## ([1-7])\.` 각각 정확히 1회, 오름차순 — 누락/중복/순서 어긋남을 나열하며 FAIL
  3. **non-goals 개수**: `## 4.` 섹션 내 최상위 리스트 항목(`^- `/`^\* `) ≥ 3개
  4. **phase + 체크박스**: `## 6.` 섹션 내 `### ` phase가 ≥ 1개, 각 phase 블록에 `- [ ]`/`- [x]` 체크박스 ≥ 1개
  5. **요구사항 상한**: 각 phase 블록 내 번호 리스트(`^\d+\.`) ≤ 50개
  추가로 `(가정)`/`(assumption)` 포함 줄을 전부 줄 번호와 함께 `ASSUMPTIONS (N):` 아래 나열 (항상 정보성, 실패 사유 아님). 5개 체크 전부 통과 시 exit 0, 하나라도 실패 시 exit 1. 인자 없음/파일 없음 → 사용법 안내 후 exit 2.

- [x] **Step 3: 테스트 통과 확인**

Run: `cd skills/prd-maker/scripts && python3 test_validate_prd.py`
Expected: 전체 테스트 `OK`.

- [x] **Step 4: SKILL.md Step 4 갱신** — 셀프 리뷰 전에 `PRD.md` 저장 → 린터 실행(`python3 ${CLAUDE_PLUGIN_ROOT}/skills/prd-maker/scripts/validate_prd.py PRD.md`) → FAIL 수정 후 재실행 (3회 시도 후에도 실패하면 리포트를 사용자에게 보여주고 진행 방향을 묻는다) → 그 다음 `quality-rules.md`로 의미적 룰만 1회 점검하도록 순서 변경.

- [x] **Step 5: quality-rules.md에 범위 주석 추가** — 첫 문단 뒤에 "구조적 체크는 `scripts/validate_prd.py`가 자동화하며 SKILL.md Step 4가 먼저 실행한다 — 이 체크리스트는 스크립트가 판단할 수 없는 의미적 판단만 다룬다" 취지의 문장 추가.

- [x] **Step 6: 문서 동기화** — 이 플랜 파일의 Task 2/Task 5 코드 블록과 파일 구조 트리, 그리고 `docs/superpowers/specs/2026-07-06-prd-maker-design.md` §3/§6을 갱신된 SKILL.md·quality-rules.md와 일치시킨다.

- [x] **Step 7: 전체 검증**

Run:
```bash
cd skills/prd-maker/scripts && python3 test_validate_prd.py
wc -l ../SKILL.md
```
Expected: 테스트 전부 통과, SKILL.md 150줄 이하.

- [x] **Step 8: Commit**

```bash
git add -A
git commit -m "feat: PRD 구조 린터 추가 (validate_prd.py + 테스트, Step 4 연동)"
```
