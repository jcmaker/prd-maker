...
- [x] **Step 4: SKILL.md Step 4 갱신** — 셀프 리뷰 전에 `PRD.md` 저장 → 린터 실행(`python3 ${CLAUDE_PLUGIN_ROOT}/skills/prd-maker/scripts/validate_prd.py PRD.md`) → FAIL 수정 후 exit 0까지 재실행 → 그 다음 `quality-rules.md`로 의미적 룰만 1회 점검하도록 순서 변경.

- [ ] **Step 5: quality-rules.md에 범위 주석 추가** — 첫 문단 뒤에 "구조적 체크는 `scripts/validate_prd.py`가 자동화하며 SKILL.md Step 4가 먼저 실행한다 — 이 체크리스트는 스크립트가 판단할 수 없는 의미적 판단만 다룬다" 취지의 문장 추가.

- [ ] **Step 6: 문서 동기화** — 이 플랜 파일의 Task 2/Task 5 코드 블록과 파일 구조 트리, 그리고 `docs/superpowers/specs/2026-07-06-prd-maker-design.md` §3/§6을 갱신된 SKILL.md·quality-rules.md와 일치시킨다.
